...
(function() {
  ...
  resultsTemplate = "<div class='api-mate-results'>     <div class='api-mate-links'>       {{#urls}}         <div class='api-mate-link-wrapper'>           <div class='api-mate-link {{urlClass}}' data-api-mate-call='{{name}}'>             <a class='label' href='{{url}}' target='_blank'>GET</a>             <a href='#' data-url='{{url}}' class='tooltipped label'                title='Send \"{{name}}\" using a POST request'                data-api-mate-post='{{name}}'>POST</a>             <a href='#' data-url='{{url}}' class='tooltipped label'                title='Show a QR code to open this link on another device'                data-api-mate-qrcode='{{name}}'>QR</a>             <span class='api-mate-method-name'>{{description}}</span>             <a href='{{url}}' target='_blank'>{{url}}</a>           </div>         </div>       {{/urls}}     </div>     <div class='api-mate-result-title'>       <h5 class='label-title'>Results {{title}}:</h5>     </div>   </div>";
  ...
  window.ApiMate = ApiMate = (function() {
    function ApiMate(placeholders, templates) {
      ...
      this.debug = false;
      this.randomNames = false;
      this.newNameOnJoin = false;
      this.urlsLast = null;
    }
    ...
    ApiMate.prototype.start = function() {
      ...
      $("[data-api-mate-random-names]").on("click", function() {
        _this.randomNames = !$("[data-api-mate-random-names]").hasClass("active");
        _this.randomizeFullName();
        _this.generateUrls();
        _this.addUrlsToPage(_this.urls);
        return true;
      });
      $("[data-api-mate-new-name-on-join]").on("click", function() {
        _this.newNameOnJoin = !$("[data-api-mate-new-name-on-join]").hasClass("active");
        return true;
      });
      this.generateUrls();
      this.addUrlsToPage(this.urls);
      this.bindPostRequests();
      this.bindJoinNameRotation();
      this.bindQrCodes();
    ...
    ApiMate.prototype.initializeMenu = function() {
      ...
      this.randomizeFullName();
      return this.setMenuValuesFromURL();
    };

    ApiMate.prototype.randomizeFullName = function() {
      var fullName;
      fullName = this.randomNames && (window.faker != null) ? window.faker.person.fullName() : "User " + Math.floor(Math.random() * 10000000).toString();
      return $("[data-api-mate-param*='fullName']").val(fullName);
    };
    ...
    ApiMate.prototype.bindJoinNameRotation = function() {
      var _this = this;
      return $(document).on('click', ".api-mate-link[data-api-mate-call='join'] a[target='_blank']", function() {
        setTimeout(function() {
          return _this.rotateFullNameIfEnabled();
        }, 0);
        return true;
      });
    };

    ApiMate.prototype.rotateFullNameIfEnabled = function() {
      if (this.newNameOnJoin) {
        this.randomizeFullName();
        this.generateUrls();
    ...
    ApiMate.prototype.bindQrCodes = function() {
      var modal, qrCodeCall, _apiMate;
      _apiMate = this;
      modal = this.placeholders['qrcode'];
      qrCodeCall = null;
      $(document).on('click', 'a[data-api-mate-qrcode]', function(e) {
        var $target, url;
        $target = $(this);
        url = $target.attr('data-url');
        qrCodeCall = $target.attr('data-api-mate-qrcode');
        $('.modal-header h4', modal).text($target.siblings('.api-mate-method-name').text());
        $('.modal-body', modal).html(window.renderQrCodeSVG(url, {
          border: 4
        }));
        $(modal).modal({
          show: true
      ...
      return $(modal).on('hidden.bs.modal', function() {
        if (qrCodeCall === 'join') {
          _apiMate.rotateFullNameIfEnabled();
        }
        return qrCodeCall = null;
      });
    };
